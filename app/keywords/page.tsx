...
const KEYWORDS = [
  { id: "k1", keyword: "ai marketing software", matchType: "Exact", campaign: "Brand Search — India", impressions: 12400, clicks: 620, ctr: 5.0, cpc: 8.2, conversions: 48, cpa: 105.8, qs: 9, status: "Active" },
  { id: "k2", keyword: "paid ads automation", matchType: "Phrase", campaign: "Brand Search — India", impressions: 8900, clicks: 356, ctr: 4.0, cpc: 12.4, conversions: 28, cpa: 157.4, qs: 8, status: "Active" },
  ...
  { id: "k5", keyword: "linkedin campaign manager alternative", matchType: "Phrase", campaign: "Brand Search — India", impressions: 6700, clicks: 268, ctr: 4.0, cpc: 15.2, conversions: 18, cpa: 226.2, qs: 7, status: "Active" },
  ...
  { id: "k7", keyword: "ad optimization platform", matchType: "Phrase", campaign: "Performance Max — E-Commerce", impressions: 22000, clicks: 990, ctr: 4.5, cpc: 9.8, conversions: 86, cpa: 112.8, qs: 9, status: "Active" },
  { id: "k8", keyword: "marketing automation tools india", matchType: "Phrase", campaign: "Brand Search — India", impressions: 7200, clicks: 216, ctr: 3.0, cpc: 14.3, conversions: 14, cpa: 220.7, qs: 6, status: "Active" },
];
...
const SUGGESTIONS = [
  { keyword: "ai bid optimization", volume: "1K–10K", competition: "Medium", estCPC: 14.2, projROI: "+28%", qs_potential: 8 },
  { keyword: "automated campaign management", volume: "1K–10K", competition: "High", estCPC: 22.6, projROI: "+18%", qs_potential: 7 },
  { keyword: "smart bidding software", volume: "100–1K", competition: "Low", estCPC: 9.4, projROI: "+34%", qs_potential: 9 },
  { keyword: "roas optimization tool", volume: "100–1K", competition: "Medium", estCPC: 18.8, projROI: "+22%", qs_potential: 7 },
];

const LANDING_PAGES = [
  { adGroup: "Brand — Exact", url: "https://admind.ai/", lcp: 1.8, fid: 42, cls: 0.04, kwRelevance: 94, hasCTA: true, mobile: 98, status: 200, lastCheck: "10 min ago" },
  { adGroup: "Competitor — Phrase", url: "https://admind.ai/vs-competitors", lcp: 2.9, fid: 88, cls: 0.12, kwRelevance: 71, hasCTA: true, mobile: 84, status: 200, lastCheck: "10 min ago" },
  { adGroup: "RLSA — Site Visitors", url: "https://admind.ai/retarget", lcp: 1.4, fid: 31, cls: 0.02, kwRelevance: 88, hasCTA: true, mobile: 96, status: 200, lastCheck: "10 min ago" },
  { adGroup: "Generic — Broad", url: "https://admind.ai/features", lcp: 3.6, fid: 142, cls: 0.18, kwRelevance: 58, hasCTA: false, mobile: 72, status: 200, lastCheck: "10 min ago" },
  { adGroup: "Product — Exact", url: "https://admind.ai/pricing", lcp: 2.1, fid: 58, cls: 0.07, kwRelevance: 82, hasCTA: true, mobile: 91, status: 200, lastCheck: "10 min ago" },
  { adGroup: "DSA — Category", url: "https://admind.ai/blog/google-ads-tips", lcp: 4.2, fid: 196, cls: 0.24, kwRelevance: 44, hasCTA: false, mobile: 68, status: 200, lastCheck: "10 min ago" },
  { adGroup: "Display — Retarget", url: "https://admind.ai/signup", lcp: 1.6, fid: 28, cls: 0.03, kwRelevance: 91, hasCTA: true, mobile: 97, status: 200, lastCheck: "10 min ago" },
  ...
  return (
    <div className="flex flex-col flex-1">
      ...
      <div className="flex-1 p-6 overflow-y-auto space-y-5">
        ...
        <div className="flex gap-1 bg-white/4 rounded-lg p-1 border border-white/8 w-fit">
          {TABS.map((t) => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-4 py-2 rounded text-xs font-medium transition-colors ${tab === t ? "bg-white/10 text-white" : "text-slate-400 hover:text-slate-300"}`}>
              {t}
            </button>
          ))}
        </div>
            ...
            <div className="px-5 pt-5 pb-3">
              <div className="relative w-64">
                ...
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search keywords..."
                  className="w-full bg-white/5 border border-white/10 rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-blue-500/60" />
              </div>
            </div>
              ...
              <tbody>
                {filtered.map((k) => (
                  <tr key={k.id} className="border-b border-white/5 hover:bg-white/3 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-300">{k.impressions.toLocaleString("en-IN")}</td>
                    <td className="px-4 py-3">
                      <span className={k.ctr > 3 ? "text-emerald-400" : k.ctr < 2 ? "text-red-400" : "text-slate-300"}>
                        {formatPct(k.ctr)}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-slate-300">₹{k.cpc}</td>
                    <td className="px-4 py-3 text-slate-300">{k.conversions}</td>
                    <td className="px-4 py-3 text-slate-300">{k.cpa > 0 ? `₹${k.cpa}` : "—"}</td>
                    <td className="px-4 py-3">
        ...
        {tab === "Negative Queue" && (
          <Card>
            <CardHeader title="Negative Keyword Queue" subtitle="Auto-flagged from Search Term Analysis — spend > 1.5x CPA, 0 conversions" />
            <div className="space-y-2">
              {negQueue.map((n) => (
                <div key={n.term} className="flex items-center gap-3 p-3 rounded-lg bg-white/3 border border-white/6">
              ...
              <tbody>
                {SUGGESTIONS.map((s, i) => (
                  <tr key={i} className="border-b border-white/5">
                    <td className="py-3 pr-4 text-white font-medium">{s.keyword}</td>
                    <td className="py-3 pr-4 text-slate-300">{s.volume}</td>
                    ...
                    <td className="py-3 pr-4 text-slate-300">₹{s.estCPC}</td>
        ...
        {tab === "QS Heatmap" && (
          <Card>
            <CardHeader title="Quality Score Heatmap" subtitle="All active keywords — darker = higher QS" />
            <div className="grid grid-cols-4 gap-2">
              {KEYWORDS.map((k) => (
                <div key={k.id} className="p-3 rounded-lg border border-white/8"
                  style={{ background: `${qsBg(k.qs)}10` }}>
                  <div className="flex items-start justify-between mb-1">
                    <span className={`text-lg font-black ${qsColor(k.qs)}`}>{k.qs}</span>
                    <span className="text-[10px] text-slate-500">/10</span>
                  </div>
                  <p className="text-xs text-slate-300 truncate">{k.keyword}</p>
        ...
        {tab === "Landing Pages" && (
          <>
            ...
            <Card className="p-0 overflow-hidden">
              <div className="px-5 pt-5 pb-3">
                <h3 className="text-sm font-semibold text-white">Landing Page Audit</h3>
                <p className="text-xs text-slate-500 mt-0.5">Automated weekly audit — Core Web Vitals, keyword relevance, CTA detection, mobile score</p>
              </div>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {LANDING_PAGES.map((p, i) => (
                      <tr key={i} className={`border-b border-white/5 hover:bg-white/3 transition-colors ${p.status !== 200 ? "opacity-60" : ""}`}>
                        <td className="px-4 py-3 text-white font-medium">{p.adGroup}</td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          {p.status !== 200 ? <span className="text-slate-600">—</span> : (
            ...
            <div className="grid grid-cols-3 gap-3 text-[11px]">
              <div className="p-3 rounded-lg bg-white/3 border border-white/6">
                <p className="font-semibold text-white mb-1 flex items-center gap-1"><Zap size={11} className="text-amber-400" /> LCP Target: ≤ 2.5s</p>
                <p className="text-slate-400">Largest Contentful Paint. Measures loading performance. Pages above threshold lose ad rank.</p>
              </div>
              <div className="p-3 rounded-lg bg-white/3 border border-white/6">
                <p className="font-semibold text-white mb-1 flex items-center gap-1"><MonitorSmartphone size={11} className="text-blue-400" /> Mobile Score: ≥ 90</p>
                <p className="text-slate-400">Google mobile-friendly test. Below 75 triggers Quality Score penalty on mobile devices.</p>
              </div>
              <div className="p-3 rounded-lg bg-white/3 border border-white/6">
                <p className="font-semibold text-white mb-1 flex items-center gap-1"><Globe size={11} className="text-purple-400" /> KW Relevance: ≥ 80%</p>
