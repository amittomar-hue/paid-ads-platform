...
const PREVIEW_DATA = [
  { campaign: "Brand Search — India", platform: "google", spend: "₹38,400", impressions: "18,400", clicks: "920", ctr: "5.0%", conversions: "74", cpa: "₹519", roas: "4.2x" },
  { campaign: "Competitor Keywords", platform: "google", spend: "₹28,400", impressions: "24,200", clicks: "484", ctr: "2.0%", conversions: "20", cpa: "₹1,420", roas: "2.1x" },
  { campaign: "Performance Max", platform: "google", spend: "₹44,800", impressions: "63,200", clicks: "2,232", ctr: "3.5%", conversions: "160", cpa: "₹280", roas: "4.8x" },
  { campaign: "SaaS Decision Makers", platform: "linkedin", spend: "₹22,400", impressions: "28,000", clicks: "784", ctr: "2.8%", conversions: "32", cpa: "₹700", roas: "3.2x" },
  { campaign: "Retargeting — All", platform: "linkedin", spend: "₹11,200", impressions: "14,600", clicks: "438", ctr: "3.0%", conversions: "22", cpa: "₹509", roas: "3.6x" },
];
...
export default function ReportsPage() {
  ...
  return (
    <div className="flex flex-col flex-1">
      <TopBar title="Custom Report Builder" subtitle="Select metrics, dimensions, and date ranges — export as CSV or PDF" />
        ...
        <div className="flex gap-1 bg-white/4 rounded-lg p-1 border border-white/8 w-fit">
          {(["builder", "saved"] as const).map((v) => (
            <button key={v} onClick={() => setView(v)}
              className={`px-4 py-2 rounded text-xs font-medium transition-colors capitalize flex items-center gap-1.5 ${view === v ? "bg-white/10 text-white" : "text-slate-400 hover:text-slate-300"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-1.5">
                      {r.metrics.slice(0, 5).map((m) => (
                        <span key={m} className="text-[10px] bg-white/6 text-slate-400 px-1.5 py-0.5 rounded">{ALL_METRICS.find((x) => x.id === m)?.label}</span>
                      ))}
                        ...
                        {ALL_METRICS.filter((m) => m.group === group).map((m) => {
                          ...
                          return (
                            <button key={m.id} onClick={() => toggleMetric(m.id)}
                              className={`flex items-center gap-2 p-2.5 rounded-lg border text-xs transition-colors ${selected ? "border-blue-500/40 bg-blue-500/10 text-blue-300" : "border-white/8 bg-white/3 text-slate-400 hover:border-white/15"}`}>
                    ...
                    {ALL_DIMENSIONS.map((d) => {
                      ...
                      return (
                        <button key={d.id} onClick={() => toggleDimension(d.id)}
                          className={`flex items-center gap-2 p-2.5 rounded-lg border text-xs transition-colors ${selected ? "border-purple-500/40 bg-purple-500/10 text-purple-300" : "border-white/8 bg-white/3 text-slate-400 hover:border-white/15"}`}>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <select value={dateRange} onChange={(e) => setDateRange(e.target.value)}
                        className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-blue-500/60">
                        <option>Today</option>
                      ...
                      <div className="flex gap-1 bg-white/4 rounded-lg p-1 border border-white/8">
                        {(["all", "google", "linkedin"] as const).map((p) => (
                          <button key={p} onClick={() => setPlatform(p)}
                            className={`flex-1 py-1.5 rounded text-xs font-medium transition-colors capitalize ${platform === p ? "bg-white/10 text-white" : "text-slate-400 hover:text-slate-300"}`}>
                    ...
                    <div>
                      ...
                      <input className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-blue-500/60"
                ...
                <Card>
                  ...
                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="text-white font-medium">{selectedMetrics.length} selected</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Dimensions</span>
                      <span className="text-white font-medium">{selectedDimensions.length} selected</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-white font-medium">{filteredPreview.length} campaigns</span>
                    </div>
                  </div>
              ...
              <Card className="p-0 overflow-hidden">
                <div className="px-5 pt-5 pb-3 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">{dateRange} · {filteredPreview.length} rows</p>
                  </div>
                    ...
                    <tbody>
                      {filteredPreview.map((row, i) => (
                        <tr key={i} className="border-b border-white/5 hover:bg-white/3 transition-colors">
                          <td className="px-4 py-3 text-white font-medium truncate max-w-[200px]">{row.campaign}</td>
                          <td className="px-4 py-3"><PlatformBadge platform={row.platform as "google" | "linkedin"} /></td>
                          {selectedMetrics.includes("spend") && <td className="px-4 py-3 text-slate-300">{row.spend}</td>}
                          {selectedMetrics.includes("impressions") && <td className="px-4 py-3 text-slate-300">{row.impressions}</td>}
                          {selectedMetrics.includes("clicks") && <td className="px-4 py-3 text-slate-300">{row.clicks}</td>}
                          {selectedMetrics.includes("ctr") && <td className="px-4 py-3"><span className={parseFloat(row.ctr) > 3 ? "text-emerald-400" : "text-slate-300"}>{row.ctr}</span></td>}
                          {selectedMetrics.includes("conversions") && <td className="px-4 py-3 text-slate-300">{row.conversions}</td>}
                          {selectedMetrics.includes("cpa") && <td className="px-4 py-3 text-slate-300">{row.cpa}</td>}
                          {selectedMetrics.includes("roas") && <td className="px-4 py-3"><span className={parseFloat(row.roas) >= 4 ? "text-emerald-400 font-semibold" : "text-slate-300"}>{row.roas}</span></td>}
                        </tr>
                      ))}
                    </tbody>
                  </table>
