...
export default function SettingsPage() {
  ...
  const [schedules, setSchedules] = useState([
    ...
    { id: "s2", name: "Daily Spend Summary", frequency: "Daily", day: "—", time: "07:00", recipients: "finance@company.com", format: "CSV", active: true },
          ...
          <div className="flex gap-1 bg-white/4 rounded-lg p-1 border border-white/8">
            {TABS.map((t) => (
              <button key={t} onClick={() => setActiveTab(t)}
                className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${activeTab === t ? "bg-white/10 text-white" : "text-slate-400 hover:text-slate-300"}`}>
                {t}
              </button>
            ))}
          </div>
          ...
          {activeTab === "API Connections" && (
            <div className="space-y-4">
              ...
              <Card>
                ...
                <div className="space-y-3">
                  <Input label="API Key" type="password" value={anthropicKey} onChange={(e) => setAnthropicKey(e.target.value)} placeholder="sk-ant-..." hint="Set ANTHROPIC_API_KEY in your .env.local file — never expose this client-side" />
              ...
              <div className="space-y-4">
                <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-500/20">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <p className="text-sm font-medium text-white">Model: claude-sonnet-4-6</p>
                  </div>
            ...
            <Card>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <label key={i} className="flex items-center justify-between p-3 rounded-lg bg-white/3 border border-white/6 cursor-pointer hover:bg-white/5 transition-colors">
            ...
            <div className="space-y-4">
              <Card>
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="text-sm font-semibold text-white">Scheduled Report Delivery</h3>
                ...
                <div className="space-y-3">
                  {schedules.map((s) => (
                    <div key={s.id} className={`p-4 rounded-lg border ${s.active ? "border-white/10 bg-white/3" : "border-white/5 bg-white/1 opacity-60"}`}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-start gap-3 flex-1">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-3 mt-1 text-[11px] text-slate-400">
                              <span>{s.frequency}{s.day !== "—" ? ` · ${s.day}` : ""} at {s.time}</span>
                              <span>→ {s.recipients}</span>
                              <span className="bg-white/8 px-1.5 py-0.5 rounded text-[10px]">{s.format}</span>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <button
                            ...
                            className={`w-8 h-4 rounded-full transition-colors relative ${s.active ? "bg-emerald-500" : "bg-white/15"}`}
                          >
              ...
              <Card>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-blue-500/60">
          ...
          {activeTab === "Deploy Guide" && (
            <div className="space-y-4">
              <Card>
                ...
                <div className="bg-[#111116] rounded-lg p-4 font-mono text-xs space-y-1">
              ...
              <Card>
                ...
                <div className="space-y-2 text-xs text-slate-300">
                  {[
                    ...
                    "4. Deploy — your Next.js API routes handle all sensitive calls server-side",
                    "5. Set ANTHROPIC_API_KEY — AI features activate automatically",
                  ].map((step) => (
                    <p key={step} className="flex gap-2"><span className="text-blue-400 flex-shrink-0">→</span>{step}</p>
                  ))}
                </div>
              </Card>
            </div>
